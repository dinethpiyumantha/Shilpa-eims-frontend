<template>
  <div id="show-notice">
    ...
    <div
      v-for="(notice, index) in filteredheder"
      :key="notice.index"
      class="single-notice"
    >
      <h4 v-rainbow class="align-center">{{ notice.heder }}</h4>
      <div class="d-flex justify-content-between">
        <article>{{ notice.body | snippet }}</article>
        <br />
        <div class="d-flex flex-column bd-highlight mb-3">
          ...
          <div class="p-2 bd-highlight">
            <button
              class="btn my-0 py-0"
              v-bind:id="notice.index"
              @click="deleteNote(notice)"
            >
              <i class="fa fa-trash" />
            </button>
          </div>
                ...
                <h5 class="modal-title" id="staticBackdropLabel">
                  <div class="d-flex">
                    <div class="flex-shrink-0">
                      <img v-bind:src="'http://localhost:8000/'+notices[noticesUpdate].image"  width="160" height="140"/>
                    </div>
                    <div class="flex-grow-1 ms-3">
                      <h2>{{ notices[noticesUpdate].heder }}</h2>
                ...
                <form class="card-body" v-on:submit.prevent="submit"  tabindex="-1" aria-labelledby="..." aria-hidden="true">
                  <blockquote class="blockquote mb-0">
                    <div class="mb-3">
                      <input
                        type="text"
                        v-model="notices[noticesUpdate].heder"
                        v-model.trim="$v.noticeHader.$model"
                        :class="{
                          'is-invalid': validationStatus($v.noticeHader),
                        }"
                        class="form-control"
                    ...
                    <div class="mb-3">
                      <textarea
                        v-model="notices[noticesUpdate].body"
                        v-model.trim="$v.noticeBody.$model"
                        :class="{
                          'is-invalid': validationStatus($v.noticeBody),
                        }"
                        class="form-control"
...
export default {
  ...
  methods: {
    ...
    submit: function () {
      ...
      this.$http.put('http://localhost:8000/api/editNotice/'
      +this.notices[this.noticesUpdate].id,notifications)
    ...
    deleteNote(notice) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete("http://localhost:8000/api/deleteNotice/" + notice.id)
